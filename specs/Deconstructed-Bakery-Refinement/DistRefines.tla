  ...
  <2>5. ASSUME NEW self \in Procs,
    ...
    <3>3. ASSUME NEW i \in Procs, number'[i] = 0, NEW j \in OtherProcs(i)
      ...
      <4>3. CASE i # self /\ j # self
        \* FIXME two occurs of Range(f) get encoded with distinct ops
        \* due to reduction to FOL -> optimize, treat global vars as free vars
  ...
  <2>10. ASSUME NEW self \in Procs, NEW oth \in Procs \ {self},
    ...
    <3>2. CASE v = ack
      ...
      <4>2. ASSUME NEW i \in Procs, number[i] = 0, NEW j \in OtherProcs(i)
        ...
        <5>3. q'[j][i] = q[j][i]
          \* FIXME Z3 needs the two additionnal facts;
          \* CVC4 manages without but takes more time
          \* Debug with an axiom profiler
          \*BY <5>1, 
          \*i \in OtherProcs(j),
          \*self \in OtherProcs(oth),
          \*POP_except DEF OtherProcs
          BY <5>1, POP_except DEF OtherProcs
        <5>. QED  BY <5>1, <5>2, <5>3
    ...
    <3>. QED  \*BY <3>2, <3>3, <3>4 DEF FullTypeOK, POP, PFunc, OtherProcs
      \* FIXME works with the additionnal fact; debug with axiom profiler
      BY <3>2, <3>3, <3>4, q[oth][self] \in Seq(Nat \cup {ack}) \ {<<>>} DEF FullTypeOK, POP, PFunc, OtherProcs
  <2>11. CASE UNCHANGED vars
...
<1>1. Init => Dec!Init
  ...
  <2>2. DecLN = [i \in Procs |-> [j \in OtherProcs(i) |-> 0]]
    \* FIXME don't know what the pb is
  ...
  <2>10. ASSUME NEW self \in Procs, NEW oth \in OtherProcs(self),
      ...
      <4>1. ASSUME NEW i \in Procs, NEW j \in OtherProcs(i) 
        ...
        <5>1. CASE i = self /\ j = oth
          ...
          <6>4. Range(q'[j][i]) \cup {ack} = Range(q[j][i])
            \* FIXME works with additional fact
            \*BY <3>1, <5>1, <6>1, RangeHeadTail, q[j][i] \in Seq(Nat \cup {ack}) \ {<<>>}
    ...
    <3>2. CASE v = 0 /\ number[oth] = 0
      ...
      <4>2. DecLN[self][oth] = qm
        \* FIXME works with additional fact
        \*BY <3>2, POP_access, RangeHeadTail, q[oth][self] \in Seq(Nat \cup {ack}) \ {<<>>} DEF DecLN, OtherProcs
      ...
      <4>2. UNCHANGED DecLN
        ...
        <5>1. CASE i = self /\ j = oth
          ...
          <6>6. number[oth] \in Range(q[oth][self])
            \* FIXME works with additional fact
            \*BY <3>3, <4>1, <6>3, <6>4, <6>5, RangeHeadTail, q[oth][self] \in Seq(Nat \cup {ack}) \ {<<>>}
...
\* Last modified Tue Jan 18 08:52:03 CET 2022 by adefourn
